...
type txBacklogMsg struct {
	...
	needRetest        bool                          // determine if we've attemted to test the transaction using the txpool and the txpool was able to verify the transaction at the time.
}
...
func (handler *TxHandler) backlogWorker() {
	...
	for {
		...
		case wi, ok := <-handler.postVerificationQueue:
			...
			if wi.needRetest {
				if err := handler.txPool.Test(wi.unverifiedTxGroup, true); err != nil {
...
func (handler *TxHandler) checkAlreadyCommitted(tx *txBacklogMsg) (processingDone bool) {
	...
	err := handler.txPool.Test(tx.unverifiedTxGroup, false)
	switch err {
	case nil:
		// do nothing.
	case pools.ErrTransactionPoolWaitingForNewBlock:
		tx.needRetest = true
	default:
		logging.Base().Debugf("txPool rejected transaction: %v", err)
		return true
