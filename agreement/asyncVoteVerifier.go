...
func (avv *AsyncVoteVerifier) verifyVote(verctx context.Context, l LedgerAccountReader, uv unauthenticatedVote, index int, message message, out chan<- asyncVerifyVoteResponse) {
	select {
...
func (avv *AsyncVoteVerifier) verifyEqVote(verctx context.Context, l LedgerAccountReader, uev unauthenticatedEquivocationVote, index int, message message, out chan<- asyncVerifyVoteResponse) {
	select {
	case <-avv.ctx.Done(): // if we're quitting, don't enqueue the request
	// case <-verctx.Done(): DO NOT DO THIS! otherwise we will lose the vote (and forget to clean up)!
	// instead, enqueue so the worker will set the error value and return the cancelled vote properly.
